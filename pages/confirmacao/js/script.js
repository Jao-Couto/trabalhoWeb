...
$(document).ready(function () {
  ...
  if (compra.forma == "PIX")
    $("#detalhes").html(
      'CPF: <span class="text-success">986.565.670-12</span>'
    );
  else if (compra.forma == "Boleto")
    $("#detalhes").html(
      '<span class="text-success">' + geraBoleto() + "</span>"
    );
  else {
    validadeA = compra.validade.split("-");
    validade = validadeA[2] + "/" + validadeA[1] + "/" + validadeA[0];
    $("#detalhes").html(
      'Número: <span class="text-success">' +
        compra.numero +
        "</span>" +
        '<br>Validade: <span class="text-success">' +
        validade +
        "</span>" +
        '<br>CVV: <span class="text-success">' +
        compra.cvv +
        "</span>"
    );
  }
